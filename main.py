...
if __name__ == '__main__':
    def check_systems():
        try:
            ...
            #  * add prompt for option to recheck any failed checks, such as connection timed out

            # TODO: add a check of factory talk activation server on the CEM server is listening; 22352
                ...
                for stm in SystemModel.find_all():
                    try:
                        with SystemConnection(stm, retry=2) as ssc:
                            ...
                            if not storage_data:
                                lg.info(
                                    f"Storage filling rate for {stm.hostname} was not found. It may not have any records.")
                            else:
                            ...
                            if fixing_str:
                                ...
                                time_diff_secs_new: float = seconds_between(datetime.datetime.now(),
                                                                            remote_system_time_new)
